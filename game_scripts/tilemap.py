from image_loader import load_image, simple_load_image

import pygame


class Tilemap(pygame.sprite.Sprite):
    def __init__(self, calculation_of_coordinates, pos, type_of_tile, varint_of_tile, sprite_group, all_sprites):
        super().__init__(sprite_group, all_sprites)
        if calculation_of_coordinates == 'tilemap':
            size_tile = [16, 16]
            increase = 2.5
            self.image =  pygame.transform.scale(simple_load_image(f"images/tiles/{type_of_tile}/{varint_of_tile}.png"), (size_tile[0] * increase, size_tile[1] * increase))
        ...
        if calculation_of_coordinates == 'offgrid':
            ...
            if type_of_tile == 'decor':
                ...
                self.rect = self.image.get_rect()
                self.rect.x, self.rect.y = pos[0] / 16 * (size_tile[0] * increase), pos[1] / 16 * (size_tile[1] * increase)
            if type_of_tile == 'large_decor':
                ...
                self.image =  pygame.transform.scale(load_image(f"images/tiles/{type_of_tile}/{varint_of_tile}.png", -1), (size_tile[0] * increase, size_tile[1] * increase))
                self.rect = self.image.get_rect()
                self.rect.x, self.rect.y = pos[0] / 16 * (size_tile[0] * increase / a), pos[1] / 16 * (size_tile[1] * increase / b)